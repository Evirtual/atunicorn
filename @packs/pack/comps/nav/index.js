...
const Nav = Actheme.create({
  ...
  Comp: (props) => {
    ...
    return (
      <Nav.Container>
        <Nav.Wrap row>
          {user && path !== '/about/' && path !== '/profile/' + id + '/about/' && <Elems.Button icon="search" iconSize="s5" onPress={() => setActive(true)} />}
          {(!user || active) && path !== '/about/' && path !== '/profile/' + id + '/about/' && <Nav.Wrap input={user}>
            <Nav.Wrap search>
              { !user
          ...
          {(user || path === '/about/' || path === '/profile/' + id + '/about/') && <Elems.Button icon="home" iconSize="s5" onPress={() => router?.push('/')} />}
          {user && id !== user.id
